...
const ProfilePageComponent = Component((): UINode => {
  ...
  useTitle(() => `${l('pixer.title')} - ${l('user.profile.title')} (${authService.user?.username})`);
  ...
  return html`
    <div class="w-screen h-screen top-0 left-0 fixed flex flex-col items-center">
      ${Header({
        title: () => l('user.profile.headerTitle'),
        button: () =>
          html`<div class="w-9 grid place-items-center cursor-pointer" @click=${logout}>
            ${Icon({
              icon: faDoorOpen,
              fill: 'rgb(55 65 81)',
              classes: 'w-8'
      ...
      <div class="flex-grow flex flex-col items-center pt-6 gap-6 w-full max-w-[430px]">
        ...
        <div class="flex gap-4 w-full px-6">
          <button
            ...
            ${Icon({ icon: faUserGroup, fill: 'rgb(55 65 81)', classes: 'w-6' })} 0 ${l('user.profile.friends')}
          </button>
          ...
          <button
            ...
            ${Icon({ icon: faEnvelope, fill: 'white', classes: 'w-6' })} 0 ${l('user.profile.requests')}
          </button>
        </div>
        ...
        <div class="flex flex-col gap-5">
          <div class="flex items-center gap-4">
            ...
            <p class="text-gray-700 text-xl">${l('user.profile.joinedOn')} <span class="font-bold">24/34/2004</span></p>
          </div>

          <div class="flex items-center gap-4">
            ${Icon({ icon: faImage, fill: 'rgb(55 65 81)', classes: 'w-6' })}
            <p class="text-gray-700 text-xl"><span class="font-bold">67</span> ${l('user.profile.uploadedImages')}</p>
          </div>

          <div class="flex items-center gap-4">
            ${Icon({ icon: faFolderClosed, fill: 'rgb(55 65 81)', classes: 'w-6' })}
            <p class="text-gray-700 text-xl"><span class="font-bold">5</span> ${l('user.profile.createdAlbums')}</p>
          </div>
        </div>
      </div>
    </div>
